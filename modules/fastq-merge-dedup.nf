process FASTQ_MERGE_DEDUP {
	...
	input:
	tuple val(meta), val(repeat), path(fastqs)
	...
	def repeatList = repeat.collect{ it.repeat }.join(' ')
	def repeatCount = repeat.size()

	if (meta.type == "ancient" && meta.merged == false && !params.discardUnmerged) // Receiving 3 files per library
		"""

		# Loop over each library (repeat), concatenate merged + unmerged reads

			for i in ${repeatList}

				do cat ${meta.id}.\$i.fastp.fq.gz ${meta.id}.\$i.fastp.um1.fq.gz ${meta.id}.\$i.fastp.um2.fq.gz > ${meta.id}.\$i.cat.fastp.fq.gz
