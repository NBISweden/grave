	...
	main:
		...
		// Help message
			...
			if (params.help) {
				...
				println (" ")
				println ("--noNestedDistance [${params.noNestedDistance}] 				If the graph is complex, distance index construction may require excessive amounts of memory. Setting true produces a limited version of the index and reduces memory imprint (true/false).")
				println (" ")
				...
				println (" ")
				println ("--duplicateTaggingPolicy [${params.duplicateTaggingPolicy}] 			Policy for tagging duplicates in the DT optional SAM/BAM field (DontTag, OpticalOnly, All). Irrelevent if --removeDuplicates is true. See Picard docs for more info.")
				...
				println ("--removeDuplicates [${params.removeDuplicates}] 				If true, Picard MarkDuplicates will not write duplicate reads to the output. If false, they will be written, with duplicate flags set (true/false).")
