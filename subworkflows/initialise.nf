	...
	main:
		...
		// Help message
			...
			if (params.help) {
				...
				println ("--graphDir [${params.graphDir}]		Path to the directory containing a '.gbz' file.")
				println ("--samplesheet [${params.samplesheet}]	Path to the samplesheet file.")
				println ("--pathsDir [${params.pathsDir}]		Path to the directory containing '.paths' files.")
				println (" ")
				...
				println (" ")
				println ("--gamDefrayEnds [${params.gamDefrayEnds}]					During GAM filtering, clip back the ends of ambiguously aligned reads (true/false). Max clip length is set by --defrayEndsLength.")
				println ("--defrayEndsLength [${params.defrayEndsLength}]				Maximum number of bases to clip from the ends of ambiguously aligned reads if --gamDefrayEnds is true (INT).")
		...
		// Nextflow version
			...
			if (!nextflow.version.matches('>=25.10.0')) {
				error ("ERROR: This workflow asks for Nextflow version '>=25.10.0'. You are running '${nextflow.version}'. Consider using the provided pixi environment.")
			}
